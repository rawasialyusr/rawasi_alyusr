  ...
  if (!isAuthorized) {
    return (
      <div style={{ height: '100vh', display: 'flex', justifyContent: 'center', alignItems: 'center', flexDirection: 'column', background: '#f8fafc' }}>
        ...
        <h1 style={{ color: '#dc2626', fontWeight: 900, marginBottom: '10px' }}>تم رفض الوصول</h1>
        <p style={{ color: '#64748b', fontWeight: 700, fontSize: '18px' }}>
          عفواً، لا تملك الصلاحيات الكافية لعرض هذه الصفحة.
        </p>
        <button 
          onClick={() => router.push('/')}
          style={{ marginTop: '20px', padding: '12px 24px', background: '#0f172a', color: 'white', borderRadius: '12px', border: 'none', cursor: 'pointer', fontWeight: 900 }}
        >
          العودة للرئيسية
        </button>
      </div>
...
fs.writeFileSync('components/authGuard.tsx', code);
