...
// 🟢 القائمة المنسدلة الذكية (SmartSelect 2.0)
const SmartSelect = ({ options, value, displayKey='name', onChange, label, hideLabel=false, placeholder="اختر..." }: any) => {
  ...
  // 🟢 فلترة ذكية تتجاهل المسافات والهمزات والتاء المربوطة
    ...
    <div style={{ position: 'relative' }} ref={ref}>
      ...
      {isOpen && (
        <div style={{ position: 'absolute', top: '100%', right: 0, left: 0, background: 'white', zIndex: 10000, border: `2px solid ${THEME.accent}`, borderRadius: '8px', boxShadow: '0 10px 30px rgba(0,0,0,0.15)', display: 'flex', flexDirection: 'column', marginTop: '5px' }}>
          ...
          {/* 🟢 ارتفاع القائمة زاد لـ 300 بيكسل مع سكرول واضح */}
  ...
  return (
    <div style={{ direction: 'rtl', minHeight: '100vh', background: '#f1f5f9', display: 'flex', fontFamily: 'Cairo, sans-serif' }}>
      <style>{`
        ...
        /* 🟢 استايلات السكرول والعناصر المحددة */
      ...
      {/* مركز العمليات (السايد بار Hover) */}
      ...
      <main className="no-print" style={{ flex: 1, padding: '40px', marginRight: isSidebarOpen ? '280px' : '70px', transition: '0.4s' }}>
        ...
        {logic.paginatedInvoices.map(inv => (
          <div key={inv.id} className="row-hover" style={{ display: 'grid', gridTemplateColumns: TABLE_GRID_LAYOUT, background: 'white', padding: '15px', borderRadius: '12px', marginBottom: '10px', alignItems: 'center', border: `1px solid ${THEME.border}`, transition: '0.2s', textAlign: 'center', backgroundColor: logic.selectedIds.includes(inv.id) ? '#fef3c7' : 'white' }}>
            ...
            <div>
               {inv.is_internal ? 
                 <span style={{ padding: '6px 12px', background: '#ffe4e6', color: THEME.ruby, borderRadius: '8px', fontSize: '12px', fontWeight: 900 }}>داخلية</span> : 
                 <span style={{ padding: '6px 12px', background: '#e0f2fe', color: '#0369a1', borderRadius: '8px', fontSize: '12px', fontWeight: 900 }}>ضريبية</span>
               }
            </div>
            ...
            <div style={{display:'flex', justifyContent: 'center', gap:'8px'}}>
               ...
               {inv.status !== 'مُعتمد' && (
                   <button onClick={() => logic.handlePostSingle(inv.id)} style={{ background: THEME.success, color: 'white', border: 'none', padding: '8px', borderRadius: '8px', cursor: 'pointer', fontWeight: 900, fontSize: '16px' }} title="ترحيل فوري">🚀</button>
               )}
            </div>
          </div>
        ))}
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(15, 23, 42, 0.9)', zIndex: 5000, display: 'flex', justifyContent: 'center', alignItems: 'center', padding: '20px', backdropFilter: 'blur(5px)' }}>
          <div style={{ background: 'white', width: '1150px', padding: '35px', borderRadius: '24px', maxHeight: '90vh', overflowY: 'auto', border: `4px solid ${THEME.accent}`, boxShadow: '0 25px 50px rgba(0,0,0,0.4)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: `3px solid ${THEME.primary}`, paddingBottom: '15px', marginBottom: '25px' }}>
                <h2 style={{ fontWeight: 900, margin: 0, color: THEME.primary }}>
                   {logic.editingId ? '✏️ تحديث المستند المالي' : '➕ إصدار مستند مالي جديد'}
                </h2>
            ...
            {/* 🟢 جدول البنود المطور (قائمة ذكية + مساحة للوصف) */}
            <div style={{ background: '#f8fafc', padding: '20px', borderRadius: '16px', border: `2px solid ${THEME.border}`, marginBottom: '20px' }}>
               <div style={{ display: 'grid', gridTemplateColumns: '1.5fr 2fr 1fr 1fr 1fr 1.2fr 50px', gap: '15px', fontWeight: 900, marginBottom: '10px', fontSize: '14px', color: THEME.primary }}>
                  <div>اختر البند من القائمة</div><div>البيان التفصيلي للفاتورة</div><div>الوحدة</div><div>الكمية</div><div>السعر</div><div>الإجمالي</div><div></div>
               </div>
               {logic.currentRecord.lines.map((l:any, i:number) => (
                 <div key={i} style={{ display: 'grid', gridTemplateColumns: '1.5fr 2fr 1fr 1fr 1fr 1.2fr 50px', gap: '15px', marginBottom: '10px', alignItems: 'start' }}>
                    ...
                    <textarea 
                       value={l.description || ''} 
                       onChange={(e)=>logic.handleLineChange(i, 'description', e.target.value)}
                       placeholder="اكتب وصف البند بالكامل هنا..."
                       style={{ width: '100%', minHeight: '42px', padding: '10px', borderRadius: '8px', border: `1px solid ${THEME.border}`, fontWeight: 900, fontSize: '14px', outline: 'none', resize: 'vertical', color: THEME.text, fontFamily: 'inherit' }}
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: '15px', alignItems: 'end' }}>
               ...
               <ModalField label="خصومات (ريال)" type="number" value={logic.currentRecord.material_discount} onChange={(v:any)=>logic.setCurrentRecord({...logic.currentRecord, material_discount:v})} />
               
               <div style={{ display: 'grid', gridTemplateColumns: 'repeat(6, 1fr)', gap: '15px', alignItems: 'end' }}>
   <ModalField label="إجمالي الأعمال" value={Number(logic.currentRecord.total_amount).toLocaleString()} readOnly />
   
   {/* 🟢 خانة نسبة ضمان الأعمال الجديدة */}
   <ModalField 
      label="ضمان أعمال (%)" 
      type="number" 
      value={logic.currentRecord.retention_percentage} 
      onChange={(v:any)=>logic.setCurrentRecord({...logic.currentRecord, retention_percentage:v})} 
   />
   ...
   <div style={{ gridColumn: 'span 1', background: THEME.primary, padding: '10px', borderRadius: '12px', color: 'white', textAlign: 'center', border: `1px solid ${THEME.accent}` }}>
...
// ... (تحديث قسم الطباعة لإظهار ضمان الأعمال)
<div className="modern-totals">
   <table style={{ width: '100%' }}>
      <tbody>
         <tr>
            <td>إجمالي الأعمال:</td>
            <td style={{textAlign: 'left'}}>{Number(pRec.total_amount).toLocaleString()} ريال</td>
         </tr>
         {/* 🟢 إظهار الضمان في الفاتورة المطبوعة */}
         <tr>
            <td>استقطاع ضمان أعمال ({pRec.retention_percentage}%):</td>
            <td style={{textAlign: 'left'}}>- {Number(pRec.retention_amount).toLocaleString()} ريال</td>
         </tr>
         <tr>
            <td>الخصومات:</td>
         ...
         <tr>
            ...
            <td style={{textAlign: 'left'}}>{Number(pRec.tax_amount).toLocaleString()} ريال</td>
         </tr>
         <tr className="modern-grand-total">
            <td style={{ padding: '15px 15px 8px 15px' }}>الصافي المستحق للدفع:</td>
            <td style={{textAlign: 'left', padding: '15px 15px 8px 15px', fontSize: '22px'}}>{Number(pRec.net_amount).toLocaleString(undefined, {minimumFractionDigits:2})} ريال</td>
         </tr>
      </tbody>
   </table>
</div>
               <ModalField label="الضريبة (15%)" value={Number(logic.currentRecord.tax_amount).toLocaleString()} readOnly />
               <div style={{ gridColumn: 'span 2', background: THEME.primary, padding: '15px', borderRadius: '12px', color: 'white', textAlign: 'center', border: `2px solid ${THEME.accent}` }}>
                  <div style={{ fontSize: '14px', opacity: 0.9, fontWeight: 900, color: THEME.accent }}>الصافي النهائي المستحق</div>
                  <div style={{ fontSize: '28px', fontWeight: 900 }}>{Number(logic.currentRecord.net_amount).toLocaleString()} <small style={{fontSize:'16px'}}>ريال</small></div>
               </div>
            </div>
          ...
          <div className="print-area modern-invoice" style={{ width: '210mm', minHeight: '297mm', background: 'white', padding: '15mm', position: 'relative', boxShadow: '0 0 30px rgba(0,0,0,0.5)' }}>
             ...
             <div className="modern-header">
                ...
                <div className="modern-title" style={{ textAlign: 'left' }}>
                    <h1 style={{ background: pRec.is_internal ? THEME.ruby : THEME.primary, color: 'white', padding: '12px 60px', borderRadius: '50px', fontSize: '26px', border: `3px solid ${THEME.accent}`, display: 'inline-block' }}>
                       {pRec.is_internal ? 'فاتورة داخلية' : 'فاتورة ضريبية'}
                    </h1>
                    <p style={{ margin: '8px 0 15px 0', fontWeight: 900, color: '#111827', fontSize: '14px', letterSpacing: '2px', textAlign: 'center' }}>
                       {pRec.is_internal ? 'INTERNAL INVOICE' : 'TAX INVOICE'}
                    </p>
                    <p style={{ fontSize: '16px', fontWeight: 900, color: '#000' }}><strong>رقم الفاتورة:</strong> #{pRec.invoice_number}</p>
                    <p style={{ fontSize: '16px', fontWeight: 900, color: '#000' }}><strong>تاريخ الإصدار:</strong> {pRec.date}</p>
                    <div style={{ marginTop: '15px', display: 'flex', justifyContent: 'flex-end' }}>
                       <ZatcaQRCode record={pRec} />
                    </div>
                </div>
             </div>
                ...
                <tbody>
                   {(pRec.invoice_lines || pRec.lines || []).map((l:any, i:number) => (
                     <tr key={i}>
                        ...
                        <td style={{textAlign: 'center', fontWeight: 900}}>{Number(l.unit_price).toLocaleString(undefined, {minimumFractionDigits:2})}</td>
                        <td style={{textAlign: 'center', fontWeight: 900, color: THEME.primary, fontSize: '16px'}}>{Number(l.total_price).toLocaleString()}</td>
                     </tr>
                   ))}
                </tbody>
             </table>
             ...
             <div className="modern-totals">
                <table style={{ width: '100%' }}>
                   <tbody>
                      <tr>
                         <td style={{color: '#111827'}}>إجمالي الأعمال:</td>
                         <td style={{textAlign: 'left', color: '#000'}}>{Number(pRec.total_amount).toLocaleString()} ريال</td>
                      </tr>
                      <tr>
                         <td style={{color: '#111827'}}>الخصومات:</td>
                         <td style={{textAlign: 'left', color: THEME.ruby}}>{Number(pRec.material_discount || 0).toLocaleString()} ريال</td>
                      </tr>
                      <tr>
                         <td style={{color: '#111827'}}>ضريبة القيمة المضافة (15%):</td>
                         <td style={{textAlign: 'left', color: '#000'}}>{Number(pRec.tax_amount).toLocaleString()} ريال</td>
                      </tr>
                      <tr className="modern-grand-total">
                         <td style={{ padding: '15px 15px 8px 15px' }}>الصافي المستحق للدفع:</td>
                         <td style={{textAlign: 'left', padding: '15px 15px 8px 15px', fontSize: '22px'}}>{Number(pRec.net_amount).toLocaleString(undefined, {minimumFractionDigits:2})} ريال</td>
             ...
             <div style={{ marginTop: '25px', fontWeight: 900, fontSize: '16px', color: '#000', background: '#f8fafc', padding: '15px', borderRadius: '8px', border: '1px solid #cbd5e1' }}>
                المبلغ بالحروف: فقط {tafqeet(Number(pRec.net_amount))}
             </div>

             <div className="modern-footer">
                <div style={{ display: 'flex', justifyContent: 'space-around', marginBottom: '40px', fontWeight: 900, color: '#000', fontSize: '16px' }}>
                ...
                <p style={{ margin: 0, color: pRec.is_internal ? THEME.ruby : '#111827', fontWeight: 900, fontSize: '14px' }}>
                   {pRec.is_internal 
                     ? '⚠️ هذه الفاتورة داخلية وغير مرتبطة بهيئة الزكاة والضريبة والجمارك 🇸🇦'
                     : '✅ تم إصدار هذه الفاتورة إلكترونياً وهي مطابقة لمواصفات هيئة الزكاة والضريبة والجمارك 🇸🇦'}
                </p>
             </div>
