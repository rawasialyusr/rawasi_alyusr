...
import RawasiSmartTable from '@/components/rawasismarttable'; // 🚀 استدعاء الجدول الذكي للتقارير

const THEME = {
  sandLight: '#F4F1EE', sandDark: '#E6D5C3', coffeeMain: '#8C6A5D',
  coffeeDark: '#43342E', goldAccent: '#C5A059', success: '#166534',
  danger: '#be123c', border: '#eef2f6'
};
...
export default function EmployeeProfilePage() {
    const { 
        ...
        filteredData, rangeKPIs, // 🚀 داتا الفلترة من اللوجيك
    ...
    // 🚀 حالات السايد بار الجديد (تثبيت + تحويم)
    ...
    // 📊 كشف الحساب المجمع (يوميات + سلف + خصومات) المفلتر
    const combinedStatement = useMemo(() => {
        ...
        const deductions = dedBase.map((d: any) => ({ ...d, dType: 'خصم مخالفة', color: THEME.danger, sign: '-' }));
    ...
    if (isLoading) return <div style={{ height: '100vh', display: 'flex', justifyContent: 'center', alignItems: 'center', background: THEME.sandLight, fontWeight: 900, color: THEME.coffeeDark }}>⏳ جاري تحميل معايير رواسي الذكية...</div>;

    return (
        <div style={{ display: 'flex', minHeight: '100vh', direction: 'rtl', fontFamily: "'Cairo', sans-serif", background: THEME.sandLight }}>
            ...
            <style>{`
                ...
                .sidebar-inner { width: 320px; padding: 40px 20px; position: relative; height: 100%; display: flex; flex-direction: column; }
                .pin-btn { position: absolute; left: 15px; top: 15px; background: none; border: none; cursor: pointer; font-size: 20px; transition: 0.3s; opacity: ${isSidebarOpen ? 1 : 0}; color: white; }

                /* ⬜ Main Viewport */
                .main-content { 
                    flex: 1; 
                    margin-right: ${isPinned ? '320px' : '85px'}; 
                    transition: margin-right 0.4s ease; 
                    padding: 40px 50px; 
                }

                /* ⚙️ Glass Modal (Centered) */
                .modal-overlay { 
                ...
                .modal-box { background: white; width: 600px; padding: 40px; border-radius: 35px; box-shadow: 0 25px 50px rgba(0,0,0,0.2); position: relative; max-height: 90vh; overflow-y: auto; }

                .sidebar-label { color: ${THEME.goldAccent}; font-size: 11px; fontWeight: 900; margin-bottom: 8px; display: block; }
                .sidebar-input { width: 100%; padding: 12px; border-radius: 12px; border: 1px solid rgba(255,255,255,0.1); background: rgba(255,255,255,0.05); color: white; margin-bottom: 20px; outline: none; font-size: 13px; }
                ...
                .tab-btn.active { color: ${THEME.coffeeDark}; border-bottom-color: ${THEME.goldAccent}; background: white; }
                
                .card-base { background: white; border-radius: 35px; box-shadow: 0 10px 40px rgba(140, 106, 93, 0.04); overflow: hidden; margin-bottom: 30px; }
                .summary-card { background: ${THEME.coffeeDark}; color: white; padding: 25px; border-radius: 25px; display: flex; justify-content: space-around; margin-bottom: 30px; border: 2px solid ${THEME.goldAccent}; }
                
                .policy-paper { background: #fcfaf8; padding: 45px; border-radius: 25px; border: 1px solid ${THEME.sandDark}; line-height: 1.8; position: relative; color: ${THEME.coffeeDark}; }
                ...
                .request-card { background: white; padding: 20px; border-radius: 20px; border: 1px solid #eef2f6; border-right: 6px solid ${THEME.goldAccent}; transition: 0.3s; margin-bottom: 15px; }
            ...
            {/* 🟦 Sidebar الجانبي الذكي */}
            <aside className="sidebar" onMouseEnter={() => setIsHovered(true)} onMouseLeave={() => setIsHovered(false)}>
                <div className="sidebar-inner">
                    <button className="pin-btn" onClick={() => setIsPinned(!isPinned)}>{isPinned ? '📍' : '📌'}</button>
                    
                    <div style={{ textAlign: 'center', marginBottom: '40px', marginTop: '20px' }}>
                        <h2 style={{ color: THEME.goldAccent, margin: 0, fontSize: isSidebarOpen ? '24px' : '18px', fontWeight: 900, transition: '0.3s' }}>
                            {isSidebarOpen ? 'رواسي اليسر' : 'RY'}
                        </h2>
                    </div>
                    ...
                    {isSidebarOpen && (
                        <div className="animate-fade-in" style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
                            <div>
                                <label className="sidebar-label">📅 عرض من تاريخ</label>
                                <input type="date" className="sidebar-input" value={searchFilters.startDate} onChange={(e) => setSearchFilters({...searchFilters, startDate: e.target.value})} />
                                
                                <label className="sidebar-label">📅 عرض إلى تاريخ</label>
                                <input type="date" className="sidebar-input" value={searchFilters.endDate} onChange={(e) => setSearchFilters({...searchFilters, endDate: e.target.value})} />

                                <label className="sidebar-label">🔍 البحث المتقدم</label>
                                <input type="text" className="sidebar-input" placeholder="اسم الموقع أو البند..." value={searchFilters.term} onChange={(e) => setSearchFilters({...searchFilters, term: e.target.value})} />
                            </div>

                            {/* 🚀 السمري الخاص بإجمالي الإنتاجية في أسفل السايد بار */}
                            <div style={{ marginTop: 'auto', padding: '20px', background: 'rgba(197, 160, 89, 0.1)', borderRadius: '20px', border: '1px solid rgba(197, 160, 89, 0.3)', textAlign: 'center' }}>
                                <div style={{ fontSize: '30px', marginBottom: '10px' }}>🏗️</div>
                                <p style={{ margin: 0, fontSize: '13px', color: THEME.goldAccent, fontWeight: 900 }}>إجمالي الإنتاجية المحققة</p>
                                <h3 style={{ margin: '5px 0 0 0', fontWeight: 900, fontSize: '28px', color: 'white' }}>
                                    {monthlyKPIs?.totalProduction || 0} <small style={{fontSize:'12px', opacity: 0.8}}>وحدة / متر</small>
                                </h3>
                            </div>
                        </div>
                    )}
            ...
            <main className="main-content">
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '40px' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '20px' }}>
                        <div style={{ width: '70px', height: '70px', borderRadius: '20px', background: 'white', border: `4px solid ${THEME.goldAccent}`, overflow: 'hidden', boxShadow: '0 10px 20px rgba(0,0,0,0.05)' }}>
                            {userProfile?.avatar_url ? <img src={userProfile.avatar_url} style={{width:'100%', height:'100%', objectFit:'cover'}} alt="P" /> : <div style={{width:'100%', height:'100%', display:'flex', alignItems:'center', justifyContent:'center', fontSize:'24px'}}>👤</div>}
                        </div>
                        <div>
                            <h1 style={{ margin: 0, color: THEME.coffeeDark, fontWeight: 900, fontSize: '28px' }}>مرحباً، {userProfile?.nickname || userProfile?.full_name} 🏗️</h1>
                            <p style={{ margin: 0, color: '#64748b', fontSize: '14px' }}>{formatDate(new Date().toISOString())} | بوابة الموظف الرقمية</p>
                        </div>
                    </div>
                    
                    <div style={{ display: 'flex', gap: '15px' }}>
                        <button onClick={() => setIsSettingsOpen(true)} style={{ background: 'white', border: '1px solid #ddd', padding: '12px 25px', borderRadius: '15px', fontWeight: 900, cursor: 'pointer', color: THEME.coffeeDark }}>⚙️ الإعدادات</button>
                        <div onClick={markAllNotificationsAsRead} style={{ position: 'relative', background: THEME.coffeeDark, color: 'white', padding: '12px 20px', borderRadius: '15px', cursor: 'pointer' }}>
                            🔔 {(notifications || []).length > 0 && <span style={{ position: 'absolute', top: '-5px', left: '-5px', background: THEME.danger, color: 'white', fontSize: '10px', width: '22px', height: '22px', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', border: '2px solid white', fontWeight: 900 }}>{notifications.length}</span>}
                        </div>
                    </div>
                </div>

                <div className="card-base">
                    {/* Navigation System - بالترتيب المطلوب */}
                    <div style={{ display: 'flex', borderBottom: '2px solid #f1f5f9', background: '#FAFAFA', overflowX: 'auto' }}>
                    ...
                    <div style={{ padding: '45px' }}>
                        ...
                        {activeTab === 'tasks' && (
                            <div className="animate-fade-in" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '35px' }}>
                                <div>
                                    <h3 style={{ color: THEME.danger, borderRight: `6px solid ${THEME.danger}`, paddingRight: '15px', marginBottom: '25px' }}>⏳ مهام قيد التنفيذ</h3>
                                    {(tasks || []).filter((t:any) => t.status !== 'completed').map((task: any) => (
                                        <div key={task.id} className="request-card" style={{ borderRightColor: THEME.goldAccent, padding: '25px' }}>
                                            ...
                                            <div style={{ display: 'flex', gap: '10px' }}>
                                                ...
                                                <button onClick={() => setFailingTaskId(task.id)} className="btn-primary" style={{ background: THEME.danger, padding: '8px 20px' }}>لم يتم</button>
                                            </div>
                                            {failingTaskId === task.id && (
                                                <div className="animate-fade-in" style={{ marginTop: '20px' }}>
                                                    <textarea style={{ width: '100%', padding: '15px', borderRadius: '12px', border: '1px solid #ddd', outline: 'none' }} placeholder="اذكر العائق الفني أو السبب بوضوح..." onChange={(e) => setFailNote(e.target.value)} />
                                    ...
                                    {(tasks || []).filter((t:any) => t.status !== 'completed').length === 0 && <p style={{color:'#94a3b8', textAlign:'center', padding: '20px', background: '#FAFAFA', borderRadius: '15px'}}>لا توجد مهام قيد التنفيذ حالياً.</p>}
                                </div>
                                <div>
                        ...
                        {/* 📑 2. تقرير العمل والإنتاجية (مربوط بـ Smart Table) */}
                        {activeTab === 'report' && (
                             <div className="animate-fade-in">
                                <RawasiSmartTable 
                                    title="📑 أرشيف يوميات العمل والإنتاجية (مفلتر)"
                                    fileName={`WorkReport_${userProfile?.nickname || 'Employee'}`}
                                    ...
                                    columns={[
                                        ...
                                        { header: 'الإنتاجية', accessor: 'daily_production', render: (val, row) => <span style={{ fontWeight: 900, color: THEME.coffeeMain }}>📦 {val || row.Prod || 0} وحدة</span> },
                        ...
                        {/* 💰 3. التقرير المالي (السلف والخصومات) */}
                        {activeTab === 'daily_fin' && (
                             <div className="animate-fade-in" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px' }}>
                                <div style={{ background: '#fcf8f1', padding: '30px', borderRadius: '25px', border: `1px solid ${THEME.goldAccent}` }}>
                                    <h4 style={{ color: THEME.goldAccent, margin: '0 0 20px 0', fontSize: '18px' }}>💵 آخر السحوبات (سلف)</h4>
                                    {(recentAdvances || []).slice(0, 5).map((a: any) => <div key={a.id} style={{ display: 'flex', justifyContent: 'space-between', padding: '12px 0', borderBottom: '1px dashed #e6d5c3', color: THEME.coffeeDark }}><span>{formatDate(a.date)}</span><span style={{ fontWeight: 900 }}>{formatCurrency(a.amount)}</span></div>)}
                                </div>
                                <div style={{ background: '#fff1f2', padding: '30px', borderRadius: '25px', border: `1px solid ${THEME.danger}` }}>
                                    <h4 style={{ color: THEME.danger, margin: '0 0 20px 0', fontSize: '18px' }}>✂️ آخر الخصومات</h4>
                                    {(recentDeductions || []).slice(0, 5).map((d: any) => <div key={d.id} style={{ display: 'flex', justifyContent: 'space-between', padding: '12px 0', borderBottom: '1px dashed #fecdd3', color: THEME.coffeeDark }}><span>{formatDate(d.date)}</span><span style={{ fontWeight: 900 }}>{formatCurrency(d.amount)}</span></div>)}
                                </div>
                            </div>
                        )}

                        {/* ⚖️ 4. الطلبات والاعتراضات */}
                        {activeTab === 'requests' && (
                             <div className="animate-fade-in">
                                ...
                                {showRequestForm ? (
                                    <div className="animate-fade-in" style={{ background: '#f8fafc', padding: '35px', borderRadius: '25px', border: `1px solid ${THEME.sandDark}` }}>
                                        ...
                                        <div style={{ display: 'flex', gap: '15px' }}>
                                            ...
                                            <button onClick={() => setShowRequestForm(false)} style={{ flex: 1, background:'#eee', border:'none', padding:'15px', borderRadius:'15px', fontWeight:900, cursor:'pointer', color: THEME.coffeeDark }}>إلغاء</button>
                                        </div>
                                    </div>
                                ) : (
                                    <div style={{ display: 'grid', gap: '20px' }}>
                                        {(userRequests || []).map((req: any) => (
                                            <div key={req.id} className="request-card" style={{ borderRightColor: req.type === 'objection' ? THEME.danger : THEME.goldAccent }}>
                                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                                                    <div>
                                                        <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '5px' }}>
                                                            <span style={{ fontSize: '10px', background: THEME.sandLight, padding: '3px 8px', borderRadius: '5px', fontWeight: 900, color: THEME.coffeeDark }}>{req.category === 'wage' ? '💰 أجور' : req.category === 'leave' ? '📅 إجازات' : '⚙️ أخرى'}</span>
                                                    ...
                                                    <span className="status-badge" style={{ background: req.status === 'approved' ? '#dcfce7' : req.status === 'rejected' ? '#ffe4e6' : '#fef3c7', color: req.status === 'approved' ? THEME.success : req.status === 'rejected' ? THEME.danger : '#b45309', padding: '6px 15px' }}>
                                                        {req.status === 'pending' ? '⏳ قيد المراجعة' : req.status === 'approved' ? '✅ تم القبول' : '❌ مرفوض'}
                                                    </span>
                                                </div>
                                                {req.admin_note && (
                                                    <div style={{ marginTop: '15px', padding: '15px', background: THEME.sandLight, borderRadius: '12px', fontSize: '13px', color: THEME.coffeeDark }}>
                                                        <strong>💬 رد الإدارة:</strong> {req.admin_note}
                                                    </div>
                                                )}
                                            </div>
                                        ))}
                                        {(userRequests || []).length === 0 && <div style={{ textAlign: 'center', padding: '50px', color: '#94a3b8', background: '#FAFAFA', borderRadius: '20px' }}>لا توجد طلبات أو اعتراضات سابقة.</div>}
                                    </div>
                                )}
                            </div>
                        )}

                        {/* 🧾 5. كشف الحساب المطور (مربوط بـ Smart Table) */}
                        {activeTab === 'statement' && (
                            <div className="animate-fade-in">
                                ...
                                {/* 🚀 الجداول الذكية تتيح التصدير للـ Excel والـ PDF بسهولة */}
                                <RawasiSmartTable 
                                    title="🧾 كشف الحساب التفصيلي"
                                    fileName={`AccountStatement_${userProfile?.nickname || 'Employee'}`}
                        ...
                        {activeTab === 'kpi' && (
                            <div className="animate-fade-in">
                                ...
                                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '20px', marginBottom: '30px' }}>
                                    <div style={{ background: 'white', padding: '25px', borderRadius: '20px', textAlign: 'center', border: `2px solid ${THEME.success}`, boxShadow: '0 5px 15px rgba(0,0,0,0.05)' }}>
                                    ...
                                    <div style={{ background: 'white', padding: '25px', borderRadius: '20px', textAlign: 'center', border: `2px solid ${THEME.goldAccent}`, boxShadow: '0 5px 15px rgba(0,0,0,0.05)' }}>
                                    ...
                                    <div style={{ background: 'white', padding: '25px', borderRadius: '20px', textAlign: 'center', border: `2px solid ${THEME.coffeeDark}`, boxShadow: '0 5px 15px rgba(0,0,0,0.05)' }}>
                                ...
                                <div className="policy-paper">
                                    ...
                                    <div style={{ display: 'grid', gap: '30px', position: 'relative', zIndex: 1 }}>
                                        ...
                                        <section style={{ background: 'rgba(197, 160, 89, 0.04)', padding: '20px', borderRadius: '15px' }}>
                                            <h4 style={{ color: THEME.coffeeMain, borderRight: `4px solid ${THEME.coffeeMain}`, paddingRight: '12px', marginBottom: '15px' }}>👷 نظام العمالة والإنتاجية</h4>
                                            <ul style={{ paddingRight: '20px' }}>
                                        ...
                                        <section style={{ border: `1px solid ${THEME.sandDark}`, padding: '20px', borderRadius: '15px' }}>
            ...
            {/* ⚙️ Glass Modal الإعدادات الشخصية (المركزية) */}
            {isSettingsOpen && (
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '30px' }}>
                            <div style={{ textAlign: 'center' }}>
                                <div style={{ width: '130px', height: '130px', borderRadius: '50%', background: THEME.coffeeDark, margin: '0 auto 20px', border: `5px solid ${THEME.goldAccent}`, overflow: 'hidden', boxShadow: '0 10px 25px rgba(0,0,0,0.15)' }}>
                                    {userProfile?.avatar_url ? <img src={userProfile.avatar_url} style={{width:'100%', height:'100%', objectFit:'cover'}} alt="Avatar" /> : <span style={{fontSize:'50px', lineHeight: '120px', color:'white'}}>👤</span>}
                                </div>
                                <input type="file" id="modalFile" hidden accept="image/*" onChange={(e) => e.target.files && uploadAvatar(e.target.files[0])} />
                                <label htmlFor="modalFile" style={{ cursor: 'pointer', color: THEME.goldAccent, fontWeight: 900, fontSize: '14px', background:'#f8fafc', padding:'12px 30px', borderRadius:'12px', border:`2px solid ${THEME.sandDark}` }}>{isSaving ? '⏳ جاري الرفع...' : '📸 تغيير الصورة الشخصية'}</label>
                            </div>
                            
                            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '25px' }}>
                                <div><label style={{ display: 'block', fontSize: '13px', fontWeight: 900, color: THEME.coffeeMain, marginBottom: '10px' }}>اللقب الوظيفي</label><input className="sidebar-input" style={{color:'black', border:'1px solid #ddd', background:'white', marginBottom:0}} defaultValue={userProfile?.nickname} onBlur={(e) => updateProfileInfo({ nickname: e.target.value })} /></div>
                                <div><label style={{ display: 'block', fontSize: '13px', fontWeight: 900, color: THEME.coffeeMain, marginBottom: '10px' }}>رقم الجوال</label><input className="sidebar-input" style={{color:'black', border:'1px solid #ddd', background:'white', marginBottom:0}} defaultValue={userProfile?.phone_number} onBlur={(e) => updateProfileInfo({ phone_number: e.target.value })} /></div>
                            </div>
                            ...
                            <button onClick={() => setIsSettingsOpen(false)} className="btn-primary" style={{width:'100%', padding: '18px', fontSize: '16px', borderRadius: '15px'}}>حفظ التغييرات وإغلاق النافذة</button>
                        </div>
                    </div>
                </div>
            )}
